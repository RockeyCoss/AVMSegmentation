...
def main():
    ...
    with tqdm(total=len(mrb_files)) as pbar:
        ...
        for index, mrb_file in enumerate(mrb_files, 1):
            ...
            nrrd_list = [i for i in zip_folder.namelist() if i.endswith('.nrrd') and
                         'c-' not in i.lower().replace(' ', '')]
            # assert len(nrrd_list) == 2, f'{nrrd_list}'
            if len(nrrd_list) != 2:
                multiple_files[mrb_file] = f'{nrrd_list}'
            else:
    ...
    for key, value in multiple_files.items():
        print(key,":", value)
